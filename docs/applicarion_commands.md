...
class ExampleCommand(application_command.GuildApplicationCommand):   
  ...
  interaction_callback_message = application_command.InteractionCallbackData(content="response", flags=64)   
  ...
  def command_function(self, interaction: application_command.Interaction, resources: ApplicationCommandResources, args: application_command.AppComArgs):   
    ...   
...
\*\* `application_command/InteractionCallbackData`クラスの**インスタンス**を設定します．**オーバーライドクラスではありません**．詳細は次のセクションを参照してください．   
  
### コールバック   
`Gateway`接続のイベントの一つである`InteractionCreate`イベントを受信した際，応答を行う必要があります．そのため，アプリケーションコマンドのインタラクションに対する応答を定義する必要があります．使用するクラスは，`application_command.InteractionCallbackData`です．   
（クラス本体は，`discord_lib2.objects.http_request.body.b_interaction.InteractionCallbackData`です）   
クラスインスタンスを作成し，`GuildApplicationCommand`,`GlobalapplicationCommand`,または`SubCommand`の`interaction_callback_message`へ渡します．   
それに併せ，`interaction_type`にコールバックタイプを指定します．   
...
```python   
command_function(interaction: Interaction, resources: ApplicationCommandResources, args: AppComArgs)   
```   
- `interaction`   
